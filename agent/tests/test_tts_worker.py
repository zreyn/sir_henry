...
def test_tts_player_init_success(
    ...
    mock_logger.info.assert_any_call(
        "Using checkpoint: ./models/test/F5TTS_v1_Base/model_1250000.safetensors"
    )
...
def test_tts_player_init_pt_checkpoint(
    ...
    mock_logger.info.assert_any_call(
        "Using checkpoint: ./models/test/F5TTS_v1_Base/model_1250000.pt"
    )
...
def test_tts_player_init_missing_ref_audio(
    ...
    mock_logger.warning.assert_called_once_with(
        "Reference file './ref/missing.wav' not found."
    )
...
def test_tts_player_warmup_success(
    ...
    with patch.object(
        TTSPlayer, "generate_audio", return_value=(np.array([0.1, 0.2]), 24000)
    ) as mock_gen:
        tts = TTSPlayer()
...
def test_tts_player_warmup_failure(
    ...
    with patch.object(
        TTSPlayer, "generate_audio", side_effect=Exception("Warmup error")
    ) as mock_gen:
        tts = TTSPlayer()
...
@patch("tts_worker.playback_audio_queue")
@patch("tts_worker.logger")
def test_tts_worker_success(
    mock_logger, mock_playback_queue, mock_interrupt, mock_sentence_queue
):
...
@patch("tts_worker.playback_audio_queue")
@patch("tts_worker.logger")
def test_tts_worker_none_audio(
    mock_logger, mock_playback_queue, mock_interrupt, mock_sentence_queue
):
...
def test_tts_worker_name_main(
    ...
    threading.Thread(
        target=audio_worker.audio_worker, daemon=True, name="AudioWorker"
    ).start()
